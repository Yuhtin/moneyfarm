package com.yuhtin.quotes.moneyfarm.sql.connection.sqlite;
...
public final class SQLiteConnection implements SQLConnection {
    ...
    public boolean configure(ConfigurationSection section) {
        this.sqlFile = createSQLFile(MoneyFarm.getInstance().getDataFolder(), section.getString("file"));
        return true;
    }
    ...
    public Connection findConnection() {
        if (connection == null) {
            try {
                ...
                t.printStackTrace();
                Bukkit.getPluginManager().disablePlugin(MoneyFarm.getInstance());
            }
        }
